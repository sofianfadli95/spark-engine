...
def getValueDictionary(data_rdd):
    ...
    key = ["created_at", "id", "id_str", "text", "truncated", "source", "is_quote_status", 
                   ...
                   "favorited","retweeted","lang", "sentiment", "total_score", "person",
                   "company", "eventNegatif", "country", "city", "lat_city", "long_city"]
    return Row(created_at=data_rdd[key[0]], id_twitter=data_rdd[key[1]], id_str_twitter=data_rdd[key[2]]
        , text=data_rdd[key[3]], truncated=bool(data_rdd[key[4]]), source=data_rdd[key[5]], is_quote_status=bool(data_rdd[key[6]])
        , quoted_status_id=data_rdd[key[7]], quoted_status_id_str=data_rdd[key[8]], retweet_count=int(data_rdd[key[9]])
        , favorite_count=int(data_rdd[key[10]]), favorited=bool(data_rdd[key[11]]), retweeted=bool(data_rdd[key[12]]), lang=data_rdd[key[13]]
        , sentiment=data_rdd[key[14]], total_score=float(data_rdd[key[15]]), person=data_rdd[key[16]], company=data_rdd[key[17]], eventNegatif=data_rdd[key[18]]
        , country=data_rdd[key[19]], city=data_rdd[key[20]], lat_city=data_rdd[key[21]], long_city=data_rdd[key[22]])
...
#rdd = parsed.foreachRDD(result)
#print(type(rdd))
parsed.count().map(lambda x:'Tweets in this batch: %s' % x).pprint()

#parsed.saveAsTextFiles("file:///D:/spark-kafka.txt")
print("Type parsed : %s " % type(parsed))
...
rdd_hive = parsed.map(lambda data_new: getValueDictionary(data_new))
rdd_hive_2 = rdd_hive.foreachRDD(result)
schemaTwitter = sqlContext.createDataFrame(rdd_hive_2)
df_writer = pyspark.sql.DataFrameWriter(schemaTwitter)
