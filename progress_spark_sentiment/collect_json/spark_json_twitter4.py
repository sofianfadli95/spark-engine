...
def collectElementJSON(data_rdd):
        ...
        if "raw" in data_rdd.keys():
            if "created_at" in data_rdd["raw"].keys():
                result["created_at"] = data_rdd["raw"]["created_at"].encode("ascii",'ignore')
            else:
                result["created_at"] = None
                                            
        if "source" in data_rdd["raw"].keys():
            result["source"] = """{}""".format(data_rdd["raw"]["source"].encode("ascii",'ignore'))
        else:
            result["source"] = None
        ...
        if "id_str" in data_rdd["raw"].keys():
            result["id_str"] = data_rdd["raw"]["id_str"].encode("ascii",'ignore')
        else:
            result["id_str"] = None
        ...
        if "text" in data_rdd["raw"].keys():
            result["text"] = """{}""".format(data_rdd["raw"]["text"].encode("ascii",'ignore'))
        else:
            result["text"] = None
                                                
        if "lang" in data_rdd["raw"].keys():
            result["language"] = data_rdd["raw"]["lang"].encode("ascii",'ignore')
        else:
        ...
        if "timestamp_ms" in data_rdd["raw"].keys():
            result["timestamp_ms"] = data_rdd["raw"]["timestamp_ms"].encode("ascii",'ignore')
        else:
        ...
        if "entities_urls" in data_rdd["raw"].keys():
            if 'urls' in data_rdd["raw"]["entities"].keys():
                try:
                    result["entities_urls"] = ",".join(data_rdd["raw"]["entities"]["urls"]).encode("ascii",'ignore')
                except TypeError:
            ...
            if "hashtags" in data_rdd["raw"]["entities"].keys():
                try:
                    result["entities_hashtags"] = ",".join(data_rdd["raw"]["entities"]["hashtags"]).encode("ascii",'ignore')
                except TypeError:
            ...
            if "symbols" in data_rdd["raw"]["entities"].keys():
                try:
                    result["entities_symbols"] = ",".join(data_rdd["raw"]["entities"]["symbols"]).encode("ascii",'ignore')
                except TypeError:
        ...
        if 'user' in data_rdd["raw"].keys():
            if "user_screen_name" in data_rdd["raw"]["user"].keys():
            ...
            if "id_str" in data_rdd["raw"]["user"].keys():
                result["used_id"] = data_rdd["raw"]["user"]["id_str"].encode("ascii",'ignore')
            else:
                result["used_id"] = None
        else:
            result["user_screen_name"] = None
