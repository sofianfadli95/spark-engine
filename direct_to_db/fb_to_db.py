...
consumer.subscribe(['enh-facebook-post'])
        ...
        try:
            try:
                try:
                    ...
                    if 'created_time' in message.value.keys():
                        created_time = message.value['created_time']
                    else:
                        created_time = None
                        
                    if 'count' in message.value['comments'].keys():
                        comments_count = message.value['comments']['count']
                    else:
                        comments_count = None
                            
                    if 'group_id' in message.value.keys():
                        group_id = message.value['group_id']
                    else:
                        group_id = None
                        
                    if 'name' in message.value['from'].keys():
                        from_name = message.value['from']['name']
                    else:
                        from_name = None
                            
                    if 'id' in message.value['from'].keys():
                        from_id = message.value['from']['id']
                    else:
                        from_id = None
                            
                    if 'id' in message.value.keys(): 
                        id_fb = message.value['id']
                    else:
                        id_fb = None
                            
                    if 'type' in message.value.keys():
                        type_fb = message.value['type']
                    else:
                        type_fb = None
                            
                    if 'likes' in message.value.keys():
                        likes_count = message.value['likes']['count']
                    else:
                        likes_count = None
                            
                    if 'status_type' in message.value.keys():
                        status_type = message.value['status_type']
                    else:
                        status_type = None
                    ...
                    if 'link' in message.value.keys():
                        link = message.value['link']
                    else:
                        link = None
                            
                    if 'message' in message.value.keys():
                        message_fb = message.value['message']
                    else:
                        message_fb = None
                    ...
                    cur.execute("""INSERT INTO FB_NLP1(CREATED_TIME,
                                                                  COMMENTS_COUNT,
                                                                  GROUP_ID,
                                                                  FROM_NAME,
                                                                  FROM_ID,
                    ...
                    comments_count, group_id, from_name, from_id, id_fb, type_fb, likes_count, status_type, name,
                    link, message_fb, positives_scores, negatives_scores, sentiment, entitas_city, entitas_person,
